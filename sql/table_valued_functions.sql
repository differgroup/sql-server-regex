...
CREATE FUNCTION dbo.RegexMatchesEx (@input nvarchar(max), @pattern nvarchar(max), @flags int = 0)
RETURNS TABLE (Position int, Match NVARCHAR(max))  
EXTERNAL NAME [RegexAssembly].[UDF].[MatchesEx] 
go
...
CREATE FUNCTION dbo.RegexSplitEx (@input nvarchar(max), @pattern nvarchar(max), @flags int = 0)
RETURNS TABLE (Position int, Match NVARCHAR(max))
EXTERNAL NAME [RegexAssembly].[UDF].[SplitEx] 
go
...
select *
from dbo.RegexMatchesEx('test test test', '\w+', 1)

select *
from dbo.RegexSplit('testtesttest', 't')

select *
from dbo.RegexSplitEx('testtesttest', 't', 1)
